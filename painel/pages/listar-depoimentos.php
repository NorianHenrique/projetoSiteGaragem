<?php
	if(isset($_GET['excluir'])){
		...
		$selectImagem = MySql::conectar()->prepare("SELECT capa FROM `tb_site.depoimentos` WHERE id = ?");
		$selectImagem->execute(array($_GET['excluir']));

		$foto = $selectImagem->fetch()['foto'];
		Painel::deleteFile($foto);
	...
	$noticias = Painel::selectAll('tb_site.depoimentos',($paginaAtual - 1) * $porPagina,$porPagina);
...
<div class="box-content">
	...
	<table>
		...
		<tr>
			...
			<td><a class="btn edit" href="<?php echo INCLUDE_PATH_PAINEL ?>editar-depoimentos?id=<?php echo $value['id']; ?>"><i class="fa fa-pencil"></i> Editar</a></td>
